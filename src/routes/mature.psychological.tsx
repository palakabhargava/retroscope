...
export const Route = createFileRoute('/mature/psychological')({
  component: MaturePsychologicalPage,
});

function MaturePsychologicalPage() {
  ...
  // Filter Psychological/Mystery drama
  const psychologicalDrama = contents.filter(c => c.genres.includes("Psychological") || c.genres.includes("Mystery") || c.genres.includes("Thriller"));
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-foreground pb-20 pt-10">
      <SEOHelper 
        title="Mind-Bending Psychological Thrillers — RetroScope Mature"
        description="Stream psychological mystery thrillers, memory puzzle stories, and intense character studies."
        canonicalPath="/mature/psychological"
      />
        ...
        <div className="border-b border-red-900/20 pb-6 mb-10 flex items-center justify-between">
          <div>
            <span className="inline-flex items-center gap-1.5 rounded-full bg-red-950/50 border border-red-800/40 px-2.5 py-0.5 font-retro text-[9px] uppercase tracking-wider text-red-500 mb-2">
              <ShieldAlert size={10} className="animate-pulse" /> Mind Terminal
            </span>
            <h1 className="font-display text-4xl font-black text-white text-glow-red">Mind-Bending Psychological</h1>
            <p className="text-sm text-zinc-400 mt-1">Intricate psychological plot twists, deep thrillers, and dark dramatic tension.</p>
          </div>
