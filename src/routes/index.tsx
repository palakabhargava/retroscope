...
import { MOVIES, MOODS, TIME_BUCKETS, moviesByMood, moviesByRuntime, type Mood } from '@/data/movies';
  ...
  return (
    <div>
        ...
        <div className="relative mx-auto flex h-full max-w-7xl items-end px-6 pb-16">
          <div className="max-w-2xl">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.5 }}
              ...
              <Link to="/search" className="rounded-sm border border-border bg-background/40 px-5 py-3 font-retro text-xs uppercase tracking-widest text-foreground backdrop-blur hover:border-primary">
      ...
      <div className="mx-auto max-w-7xl space-y-16 px-4 py-16">
        ...
        <section className="space-y-6">
          ...
          <div className="-mx-4 overflow-x-auto px-4">
            <div className="flex gap-5">
              {moviesByMood(mood).slice(0, 10).map(m => <div key={m.id} className="shrink-0"><PosterCard movie={m}/></div>)}
            </div>
          </div>
        </section>
              ...
              <div key={b.id} className="rounded-md border border-border bg-card p-5">
                ...
                <div className="mt-4 -mx-2 overflow-x-auto px-2">
                  <div className="flex gap-3">
                    {moviesByRuntime(b.max).slice(0, 8).map(m => <div key={m.id} className="shrink-0"><PosterCard movie={m} size="sm"/></div>)}
                  </div>
                </div>
              </div>
            ))}
        ...
        <MovieRow title="From the Vault" subtitle="— Curated by the projectionist —" movies={MOVIES.slice(0, 12)} />
        <MovieRow title="Late Night Programme" subtitle="— After midnight only —" movies={moviesByMood('night-vibes')} />
        <MovieRow title="Rainy Window Companions" subtitle="— Pour a coffee —" movies={moviesByMood('rainy-mood')} />
      </div>
    </div>
  );
}
